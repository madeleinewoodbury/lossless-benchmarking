...
const CompressionResultsChart = () => {
	...
	const sizeTooltip = ({ active, payload }) => {
		if (active && payload && payload.length) {
			const compressedSize = payload[0].payload.size.size_in_kb
			const displayCompressedSize =
				compressedSize === 0 ? "N/A" : `${compressedSize.toFixed(3)} KB`
			return (
	...
	const timeTooltip = ({ active, payload }) => {
		if (active && payload && payload.length) {
			const timeElapsed = payload[0].payload.time.time_in_seconds
			const displayTimeElapsed =
				timeElapsed === 0 ? "N/A" : `${timeElapsed.toFixed(3)} seconds`
			return (
				<div className="bg-black border border-gray-300 p-2 rounded shadow-lg">
					<p className="font-bold">{payload[0].payload.technique}</p>
	...
	return (
		<div className="pt-16 fl py-12 flex flex-col items-center gap-y-8 mx-auto text-white lg:flex-row lg:flex-wrap">
			<div className="w-full flex flex-col items-center lg:w-1/2">
				...
				<ResponsiveContainer width="100%" height={300}>
					<BarChart data={result}>
						...
						<Bar dataKey="size.size_in_kb" name="Size in KB">
			...
			<div className="w-full flex flex-col items-center lg:w-1/2">
				<h3 className="text-xl pb-6">Time Elapsed (s)</h3>
				<ResponsiveContainer width="100%" height={300}>
					<BarChart data={result}>
						<XAxis dataKey="label" className="hidden md:block" />
						<YAxis />
						<Tooltip content={timeTooltip} />
						<Bar dataKey="time.time_in_seconds" name="Time in seconds">
							{result.map((entry, index) => (
								<Cell key={`cell-${index}`} fill={colors[index]} />
							))}
							<LabelList dataKey="short_name" position="bottom" offset={5} />
